...
def json_object_search(obj, subreddit, word, cur):
    startDate = str(datetime.now() - timedelta(days=365))
    ...
    query = cur.execute("SELECT day, SUM(count) FROM (SELECT day, SUM(count) as count FROM reddit."+subreddit+" WHERE word ='"+word+"' GROUP BY day UNION SELECT generate_series(timestamp '" +
                        startDate+"', timestamp '"+datetime.now().strftime('%Y-%m-%d')+"', '1 day'):: DATE as day, 0) AS union_series GROUP BY union_series.day ORDER BY union_series.day LIMIT 365;")
    results = cur.fetchall()
    query = cur.execute("SELECT day, SUM(total) FROM (SELECT day, total FROM count_"+subreddit+" UNION SELECT generate_series(timestamp '"+startDate+"', timestamp '" +
                        datetime.now().strftime('%Y-%m-%d')+"', '1 day'):: DATE as day, 0) AS union_series GROUP BY union_series.day ORDER BY union_series.day LIMIT 365;")
    totals = cur.fetchall()
...
def json_object_events(obj, cur):
    query = cur.execute("SELECT id, createdate, address, incident_type, urgency, lat, lng FROM twitter.police ORDER BY createdate DESC LIMIT 15;")
    results = cur.fetchall()
